...
__m128i _simd_compute_erase(const __m128i &data_p, const int shift, const __m128i &zero, const __m128i &one, const __m128i &max_dp_shifted, const __m128i &data_c, const __m128i &data_d) {
  // data = (data * LOGO_MAX_DP + c) * (1<<30 / d');

  const __m128i data_ps = _mm_slli_epi32(data_p, shift); // shift to 16 bit
  ...
  const __m128i data_hi_pmu = _mm_mul_epu32(data_hi_pxl, max_dp_shifted); // pmu = pxl * LOGO_MAX_DP
  const __m128i data_hi_pma = _mm_add_epi32(data_hi_pmu, data_hi_c);      // pma = pmu + c
  const __m128i data_hi_pmz = _mm_max_epi32(data_hi_pma, zero);           // pmz = pma.saturate 0
  const __m128i data_hi_rtm = _mm_mul_epu32(data_hi_pmz, data_hi_d);      // rtm = pma * d
  const __m128i data_hi_rts = _mm_srli_epi64(data_hi_rtm, 30);            // rts = rtm.shift-back -2
  ...
  const __m128i result_epu = _mm_srli_epi32(result_epu16, shift);
    ...
    int64_t d = (1<<30) / data_d.m128i_i32[0];
...
template<> template<>
void DelogoEngine<ERASE_LOGO>::realProcess<uint8_t>(uint8_t* cellptr, int upbound, int* array_c, int* array_d) {
  const int shift = (16 - _ebpc);

  const __m128i zero = _mm_setzero_si128();
  const __m128i one = _mm_set1_epi64x(1);
  auto max_dp_shifted = _mm_set1_epi32(LOGO_MAX_DP << 4);
  for (int j = 0; j < upbound; j += 16) {
    ...
    data1_4 = _simd_compute_erase(data1_4, shift, zero, one, max_dp_shifted, data_c, data_d);

    data_c = _mm_stream_load_si128((__m128i *)(&array_c[j+4]));
    data_d = _mm_stream_load_si128((__m128i *)(&array_d[j+4]));
    data2_4 = _simd_compute_erase(data2_4, shift, zero, one, max_dp_shifted, data_c, data_d);

    data_c = _mm_stream_load_si128((__m128i *)(&array_c[j+8]));
    data_d = _mm_stream_load_si128((__m128i *)(&array_d[j+8]));
    data3_4 = _simd_compute_erase(data3_4, shift, zero, one, max_dp_shifted, data_c, data_d);

    data_c = _mm_stream_load_si128((__m128i *)(&array_c[j+12]));
    data_d = _mm_stream_load_si128((__m128i *)(&array_d[j+12]));
    data4_4 = _simd_compute_erase(data4_4, shift, zero, one, max_dp_shifted, data_c, data_d);
...
template<> template<>
void DelogoEngine<ERASE_LOGO>::realProcess<uint16_t>(uint16_t* cellptr, int upbound, int* array_c, int* array_d) {
  const int shift = (16 - _ebpc);
  ...
  for (int j = 0; j < upbound; j += 8) {
    ...
    data_c = _mm_stream_load_si128((__m128i *)(&array_c[j]));
    data_d = _mm_stream_load_si128((__m128i *)(&array_d[j]));
    data1_2 = _simd_compute_erase(data1_2, shift, zero, one, max_dp_shifted, data_c, data_d);

    data_c = _mm_stream_load_si128((__m128i *)(&array_c[j+4]));
    data_d = _mm_stream_load_si128((__m128i *)(&array_d[j+4]));
    data2_2 = _simd_compute_erase(data2_2, shift, zero, one, max_dp_shifted, data_c, data_d);
